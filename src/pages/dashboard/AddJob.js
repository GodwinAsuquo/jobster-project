...
const handleSubmit = (e) =>{
  ...
  if(!position || !company || !jobLocation){
   ...
   if(isEditing){
    dispatch(
      editJob({
        jobId: editJobId, job:{position, company, jobLocation, jobType, status}
      })
    )
    return;
  }
dispatch(createJob({ position, company, jobLocation, jobType, status }))
//here, we are just typing a couple of our state values that we want the server to look for
}
...
useEffect(()=>{
  ...
  return (
    <Wrapper>
      <form className="form">
        <h3>{isEditing? 'editJob' : 'addJob'}</h3>
